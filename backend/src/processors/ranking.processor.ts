...
export class RankingProcessor extends BaseProcessor {
  ...
  private getAnimationExpr(animation: string, baseY: string, delay = 0.3): string {
    switch (animation) {
      case 'slide': return `enable='gte(t,${delay})':y='${baseY}+150*max(0,1-(t-${delay})/0.4)'`;
      default: return `enable='gte(t,${delay})'`;
    }
  }
  ...
  async process(id: string, video_settings: any, items_payload: any[]): Promise<string> {
    ...
    try {
      ...
      try {
        fs.copyFileSync('C:\\Windows\\Fonts\\seguib.ttf', path.join(tempDir, localFontName));
      } catch (e) {
        try { fs.copyFileSync('C:\\Windows\\Fonts\\arialbd.ttf', path.join(tempDir, localFontName)); } catch(e2) {}
      }
      try {
        fs.copyFileSync('C:\\Windows\\Fonts\\seguiemj.ttf', path.join(tempDir, localEmojiName));
      } catch (e) {
        try { fs.copyFileSync(path.join(tempDir, localFontName), path.join(tempDir, localEmojiName)); } catch(e2) {}
      }
      ...
      for (let i = 0; i < items_payload.length; i++) {
        ...
        fs.copyFileSync(path.join(tempDir, localFontName), path.join(itemDir, localFontName));
        fs.copyFileSync(path.join(tempDir, localEmojiName), path.join(itemDir, localEmojiName));
        ...
        const endTime = item.endTime ? parseFloat(item.endTime) : startTime + 30;
        ...
        try {
          ...
          execSync(`${ffmpeg} -f lavfi -i color=c=black:s=1080x1920:d=${finalDuration} -c:v libx264 -pix_fmt yuv420p "${rawVideoPath}" -y`);
        }
        ...
        if (rank === 1) {
          ...
          items.push({ id: 'auto-drumroll', type: 'sfx', name: 'Drum Roll', startTime: 0, duration: 3, path: 'https://cdn.pixabay.com/audio/2022/01/18/audio_651a2a5371.mp3' });
        } else {
          // Add Whoosh for other ranks
          items.push({ id: 'auto-whoosh', type: 'sfx', name: 'Whoosh', startTime: 0, duration: 1, path: 'https://cdn.pixabay.com/audio/2022/03/10/audio_c35071190d.mp3' });
        }
        item.timelineItems = items;

        const clipHasAudio = this.hasAudioStream(rawVideoPath);
        let clipDuration = finalDuration;
        let inputArgs = isYouTube ? `-i "raw_clip.mp4"` : `-ss ${startTime} -t ${finalDuration} -i "raw_clip.mp4"`;
        let audioMapIdx = 0;
        ...
        if (audioMode === 'ai') {
          ...
          try {
            ...
            const durStr = execSync(`${ffprobe} -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "audio.mp3"`, { cwd: itemDir }).toString().trim();
            ...
            execSync(`${ffmpeg} -f lavfi -t ${finalDuration} -i anullsrc=sample_rate=44100 -c:a aac "silent_tts.aac" -y`, { cwd: itemDir });
          ...
          if (item.musicPath) {
            ...
            execSync(`${ffmpeg} -f lavfi -t ${finalDuration} -i anullsrc=sample_rate=44100 -c:a aac "silent_music.aac" -y`, { cwd: itemDir });
            inputArgs += ` -i "silent_music.aac"`;
            audioMapIdx = 1;
          }
        } else if (audioMode === 'clip' && !clipHasAudio) {
           execSync(`${ffmpeg} -f lavfi -t ${finalDuration} -i anullsrc=sample_rate=44100 -c:a aac "silent.aac" -y`, { cwd: itemDir });
        ...
        const groupStartY = rankY_raw !== 0 ? rankY_raw : Math.round(1920 / 2 - ((N - 1) * SPACING) / 2) - 150;
        const videoY = videoY_raw !== -1 ? videoY_raw : '(oh-ih)/2';
        const h1X = headerAlign === 'left' ? `${headerX}` : headerAlign === 'right' ? `${headerX}-text_w` : `${headerX}-text_w/2`;
        ...
        const fx = EffectsEngine.buildFilter(item.timelineItems || [], vCur, `${audioMapIdx}:a`, currentInputIdx, itemDir);
        if (fx.filter) {
        ...
        for (let j = 0; j < N; j++) {
          ...
          const jColor = jItem.rankColor ? this.hexToFFmpegColor(jItem.rankColor) : itemRankColorFFmpeg;
          ...
          if (jRank >= currentRankNumber) {
            if (jItem.clipTitle) {
               ...
               const titleColor = jItem.clipTitleColor ? this.hexToFFmpegColor(jItem.clipTitleColor) : 'white';
               const titleX = rankX + (rankFontSize * 1.1);
               const titleY = jY + (rankFontSize * 0.1);
               const strokeW = jItem.clipTitleStroke || 2;
               const strokeC = jItem.clipTitleStrokeColor ? this.hexToFFmpegColor(jItem.clipTitleStrokeColor) : '0x000000';
               const titleAnim = isActive ? this.getAnimationExpr(textAnimation, '0', 0.3) : '';
               const escapedTitle = (jItem.clipTitle || '').replace(/'/g, "'\\\\\\''");
        ...
        execSync(`${ffmpeg} ${inputArgs} -filter_complex "${filterComplex}" -map "[vout]" -map "[aout]" -t ${clipDuration} -c:v libx264 -preset fast -pix_fmt yuv420p -r 30 -c:a aac -ar 44100 "output_clip.mp4" -y`, { cwd: itemDir });
        clipPaths.push(videoPath);
        await this.supabaseService.updateJobStatus(id, 'PROCESSING', Math.min(15 + Math.floor(((i + 1)/N)*70), 85));
      }
      ...
      if (globalMusicPath) {
         ...
         execSync(`${ffmpeg} -f concat -safe 0 -i "clips.txt" -i "${safeMusicPath}" -filter_complex "[0:a]volume=1.0[a0];[1:a]volume=${volFloat}[a1];[a0][a1]amix=inputs=2:duration=first[aout]" -map 0:v -map "[aout]" -c:v libx264 -preset fast -pix_fmt yuv420p -c:a aac -ar 44100 -async 1 "final.mp4" -y`, { cwd: tempDir });
      } else {
