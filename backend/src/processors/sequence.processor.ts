  ...
  async process(id: string, video_settings: any, items_payload: any[]): Promise<string> {
    ...
    try {
      ...
      for (let i = 0; i < items_payload.length; i++) {
        ...
        // Download
        try {
          execSync(`${ytdlp} -f "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]" "${item.link}" -o "${rawVideoPath}" --force-overwrites -q`, { timeout: 300000 });
        } catch (e) {
          // Placeholder if download fails
          execSync(`${ffmpeg} -f lavfi -i color=c=black:s=1920x1080:d=5 -c:v libx264 -pix_fmt yuv420p "${rawVideoPath}" -y`);
        }

        // Standardize to 1080p 30fps for concat stability
        execSync(`${ffmpeg} -i "${rawVideoPath}" -vf "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,fps=30" -c:v libx264 -preset fast -pix_fmt yuv420p "${outputVideoPath}" -y`);
        
        clipPaths.push(outputVideoPath);
        await this.supabaseService.updateJobStatus(id, 'PROCESSING', 15 + Math.floor((i/items_payload.length)*70));
      }
      ...
      execSync(`${ffmpeg} -f concat -safe 0 -i "clips.txt" -c copy "final.mp4" -y`, { cwd: tempDir });
