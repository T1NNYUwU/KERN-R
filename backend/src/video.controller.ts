...
export class VideoController {
  ...
  async getVideoPreview(@Query('url') url: string) {
    try {
      ...
      // 1. Get metadata
      const info = execSync(`${ytdlp} --dump-json --no-playlist "${url}"`, {
        timeout: 30000,
  ...
  @Post('preview-voice')
  async previewVoice(
    @Body() body: { text: string; voice: string; engine: string },
  ) {
    ...
    try {
      ...
      if (supabaseUrl) {
        ...
        try {
          ...
          this.logger.warn(
            `Failed to cleanup local file: ${cleanupErr.message}`,
          );
        }
      }
    } catch (e) {
      this.logger.warn(
        'Failed to upload to Supabase, falling back to local URL',
      );
    }
